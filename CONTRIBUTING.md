...
practice, an upstream `ckpool` bump, since that code assembles the coinbase output
of a solved block — cut a throwaway prerelease branch:
...
`releaseRules` in `release.config.js` (only the `ckpool` and `dependencies` scopes
release):

- **Patch release** (`1.0.X`): a Dockerfile dependency bump, e.g. the Debian base
  image (`fix(dependencies): bump DEBIAN_BASE …`)
- **Minor release** (`1.X.0`): an upstream ckpool source bump
  (`feat(ckpool): …`)
...
- Example: `fix(dependencies): bump DEBIAN_BASE to bookworm-slim@sha256:…`

### Commit Types and Release Rules

Commits are categorized into two groups for release triggering. **Release
triggering is scope-gated** (enforced by `releaseRules` in `release.config.js`):
only the `ckpool` and `dependencies` scopes can produce a release. A
`feat`/`fix`/`perf` on any other scope — or with no scope — is treated as a
non-release commit, no matter its type.

**Version-bumping commits** — Only these two forms trigger a release:

- `feat(ckpool): ...` — an upstream ckpool source bump → **minor**
- `fix(dependencies): ...` — a Dockerfile dependency bump, e.g. the Debian base
  image (`ARG DEBIAN_BASE`) → **patch**
...
management. Renovate opens PRs against the **`main`** branch when new versions are
available. The commit type is chosen per update level:

| Update type | Commit produced | Release triggered |
|-------------|-----------------|-------------------|
| Upstream **ckpool commit** (`ARG CKPOOL_COMMIT`) | `feat(ckpool): ...` | minor |
| **Debian base image** digest (`ARG DEBIAN_BASE`) | `fix(dependencies): ...` | patch |
| GitHub Actions / npm | `chore(dependencies): ...` | none |

No manual version bumping is required; merge the Renovate PR after CI passes —
**except for upstream ckpool bumps**. Those are never auto-merged and carry
a `review-required` label, because that code assembles the coinbase output of a
solved block: it decides where a found block's reward is paid. Read the upstream
diff before merging one. To canary one first, route it through an on-demand `beta`
branch (see *Prerelease Canaries*).
...
The ckpool project uses a gated pipeline for quality and security:
