...
# The two inputs that define what is actually inside the image: which ckpool
# commit is compiled in, and which base image it runs on.
CKPOOL_COMMIT=$(grep -m1 'ARG CKPOOL_COMMIT=' "$DOCKERFILE" | sed 's/.*=\([^ ]*\).*/\1/')
DEBIAN_BASE=$(grep -m1 'ARG DEBIAN_BASE=' "$DOCKERFILE" | sed 's/^ARG DEBIAN_BASE=//')
# debian:bookworm-slim@sha256:... → bookworm-slim
DEBIAN_VERSION="${DEBIAN_BASE#debian:}"
DEBIAN_VERSION="${DEBIAN_VERSION%%@*}"

for var in CKPOOL_COMMIT DEBIAN_VERSION; do
    [ -n "${!var}" ] || { echo "Error: could not extract $var from $DOCKERFILE"; exit 1; }
done

SHORT_COMMIT="${CKPOOL_COMMIT:0:12}"
RELEASE_DATE=$(date +%Y-%m-%d)

if [[ "$RELEASE_VERSION" == *"beta"* ]]; then
    SECTION="### Beta Releases"
    sed -i 's/ (latest beta)//' "$README" || true
    NEW_ROW="| $RELEASE_VERSION (latest beta) | \`$SHORT_COMMIT\` | $DEBIAN_VERSION | $RELEASE_DATE |"
else
    SECTION="### Stable Releases"
    sed -i 's/ (latest)//' "$README" || true
    sed -i 's/ (latest beta)//' "$README" || true
    NEW_ROW="| $RELEASE_VERSION (latest) | \`$SHORT_COMMIT\` | $DEBIAN_VERSION | $RELEASE_DATE |"
fi
